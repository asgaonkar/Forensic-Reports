...
\begin{enumerate}
    ...
    \item $PKE_{PK_x} (K)$ is the public-key encryption, under $x$’s public key, of $K$.\footnote{RSA or ElGamal}

    \item $SIGN_{SK_x}(Z)$ is the digital signature, under $x$’s private key, of Z.\footnote{RSA or DSA}

    \item $E_{K_0}(X)$ is the symmetric encryption of $X$ under key $K_0$.\footnote{DES, IDEA, or Blowfish}. 

    \item $MAC_{K_0}(X)$ is the symmetric message authentication code under key $K_0$, of $X$. 
